...
const NewTx = (props) => {
  ...
  const onSubmit = (event) => {
    ...
    setAddModal(false)
    addTransaction({ variables: { 'user_id': userId, 'description': description, 'merchant_id': merchantId, 'debit': (txType === 'debit'), 'credit': (txType === 'credit'), 'amount': amount }, refetchQueries: [{ query: GetTransactions }] }).then(() => setRedirect(true))
  }
        ...
        <ul>
          ...
          <div css={debitCreditStyle}>
            <li>
              ...
              <input id='debit' name='type' onChange={event => setTxType(event.target.value)} type='radio' value='debit' />
            </li>
            <li>
              <label htmlFor='credit'>Credit:</label>
              <input id='credit' name='type' onChange={event => setTxType(event.target.value)} type='radio' value='credit' />
            </li>
          </div>
          <li>
